...
class SwtBridgeDataTest {

    @Test
    fun `exercise data class holds values`() {
        val ex = SwtBridge.Exercise(1, "Bench Press", "compound", "chest", "barbell")
    ...
    fun `exercise equality by value`() {
        val a = SwtBridge.Exercise(1, "Squat", "compound", "legs", "")
        val b = SwtBridge.Exercise(1, "Squat", "compound", "legs", "")
        assertEquals(a, b)
    }
    ...
    fun `exercise inequality on different id`() {
        val a = SwtBridge.Exercise(1, "Squat", "compound", "legs", "")
        val b = SwtBridge.Exercise(2, "Squat", "compound", "legs", "")
        assertNotEquals(a, b)
    }

    @Test
    fun `workout set data class holds values`() {
        val set = SwtBridge.WorkoutSet(10, 1, 5, 1, 8, 100.0, 8.0)
    ...
    fun `exercise stats data class holds values`() {
        val stats = SwtBridge.ExerciseStats(5, 120.0, 100.0, 5000.0, 10)
    ...
    fun `template set data class holds values`() {
        val ts = SwtBridge.TemplateSet(1, 2, 3, 1, 10, 50.0, 7.0)
    ...
    fun `progression point data class holds values`() {
        val pp = SwtBridge.ProgressionPoint("2024-06-15", 120.0, 100.0, 3000.0)
    ...
    fun `workout template data class holds values`() {
        val wt = SwtBridge.WorkoutTemplate(1, "Push Day", "chest and triceps", 6)
    ...
    fun `workout data class copy works`() {
        val w = SwtBridge.Workout(1, "Morning", "2024-06-15 08:00:00", "", "notes", 5)
